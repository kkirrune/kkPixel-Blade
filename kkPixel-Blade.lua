...
-- Author: kkirru-style (Gemini)
-- Version: V5.8 (Stable) — Fixes Remote Error

-- ĐÓNG GÓI TOÀN BỘ LOGIC VÀO HÀM CHÍNH ĐỂ TRÁNH LỖI 'ATTEMPT TO CALL A NIL VALUE' Ở DÒNG 1
local function safe_start_script()
    ...
    -- Lấy LocalPlayer: Dùng pcall để tránh lỗi
    local Player = Players.LocalPlayer
    if not Player and Players.LocalPlayer then Player = Players.LocalPlayer end -- Thử lại nếu bị nil lúc khai báo đầu tiên
    ...
    -- V5.8 Stable: Khai báo lại các biến logic
    ...
    local function tryHookRemote(remote)
        ...
        -- [V5.8 FIX]: Các Remote cần loại trừ vì thường chỉ là hiệu ứng (effects) hoặc gây lỗi (BoneBlade)
        local EXCLUDED_KEYWORDS = {"slash", "effect", "visual", "hitmarker", "boneblade", "tool"} 
        for _, keyword in ipairs(EXCLUDED_KEYWORDS) do
            if remoteNameLower:find(keyword) then
                return -- Bỏ qua các remote này
            end
        end
        ...
        elseif remote:IsA("RemoteFunction") then
            ...
            remote.InvokeServer = function(self, ...)
                ...
                -- Kiểm tra lại loại trừ cho RemoteFunction
                local EXCLUDED_KEYWORDS_FUNC = {"slash", "effect", "visual", "hitmarker", "boneblade", "tool"} 
                for _, keyword in ipairs(EXCLUDED_KEYWORDS_FUNC) do
                    if remoteName:find(keyword) then
                        return originalInvoke(self, table_unpack(args)) -- Vẫn cho phép gọi gốc nhưng không coi là remote tấn công
                    end
                end
    ...
    local function updateTargetCache(maxRange)
        ...
        if not localHumanoid or localHumanoid.Health <= 0 then TARGET_CACHE = nil; lastCacheUpdate = os.clock(); return nil end
        local localRoot = Player.Character and Player.Character.PrimaryPart
        if not localRoot then TARGET_CACHE = nil; lastCacheUpdate = os.clock(); return nil end
        ...
        loopConnection = RunService.Heartbeat:Connect(function(deltaTime)
            ...
            if SETTINGS.AuraEnabled and target and AttackRemote then
                if burstCounter > 0 then
                    local jitterDelay = random(SETTINGS.BurstDelay * 0.9, SETTINGS.BurstDelay * 1.1) -- Sửa thành BurstDelay cho các hits trong burst
                    ...
                    if SETTINGS.BurstEnabled and burstCharge >= 1 then
                        ...
                        local jitterBurstDelay = random(currentAttackDelay * 0.9, currentAttackDelay * 1.05) -- Sửa thành AttackDelay cho cooldown giữa các burst
    ...
    local function createUI()
        ...
        ScreenGui.Name = "PixelBlade_V5_8_UI"
        ...
        Title.Text = "PIXEL BLADE V5.8 (Stable)"
    ...
    -- Khởi tạo UI (đã đóng gói trong pcall để tránh lỗi)
    wait(0.5) -- Thêm lệnh chờ để môi trường được tải đủ
    ...
    print("-------------------------------------------------------")
    print("KKPIXEL_BLADE_V5.8: Enhanced Native UI (Stable) LOADED")
    print("BURST: " .. tostring(SETTINGS.BurstEnabled) .. " | HitboxScaleInterval: " .. tostring(SETTINGS.HitboxScaleChangeInterval))
    print("-------------------------------------------------------")
end

-- Lệnh gọi hàm chính bằng pcall để bắt bất kỳ lỗi khởi tạo nào
local success, result = pcall(safe_start_script)
if not success then
    warn("FATAL ERROR (safe_start_script failed): " .. tostring(result))
end
